...
#
# Optional:
#   RESULTS_PR_REPO             default https://github.com/Milanka00/api-platform.git
...
RESULTS_PR_REPO="${RESULTS_PR_REPO:-https://github.com/Milanka00/api-platform.git}"
...
# owner/repo for gh (strip .git and scheme)
RESULTS_PR_SLUG="$(
    python3 - <<PY
import re, os
...
short = {
    ...
    "Throughput (Requests/sec)": "TPS",
    "Average Response Time (ms)": "Avg (ms)",
    "Error %": "Err %",
...
if [[ -z "${PR_URL}" ]]; then
    PR_URL="$(gh pr create \
        ...
        --title "docs(perf): Jenkins results ${TEST_ID}" \
        --body "$(cat <<EOF
## Summary
